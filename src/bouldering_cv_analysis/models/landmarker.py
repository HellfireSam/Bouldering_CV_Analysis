"""bouldering_cv_analysis.models.landmarker

MediaPipe Pose Landmarker wrapper used to configure, initialize, and run
pose detection for images, videos, or live streams.
...
class PoseLandmarkerModel:
    ...
    def _create_landmarker(self) -> vision.PoseLandmarker:
        ...
        Returns:
            vision.PoseLandmarker: A configured MediaPipe Pose Landmarker instance.
        ...
        options = PoseLandmarkerOptions(
            ...
            min_pose_tracking_confidence=self.min_pose_tracking_confidence,
            output_segmentation_masks=self.output_segmentation_masks,
            results_callback=print_result if self.input_mode == "live_stream" else None)

        return PoseLandmarker.create_from_options(options)  

    def detect(self, image: mp.Image, frame_timestamp: int=None) -> vision.PoseLandmarkerResult:  
        """
        Run pose detection and return the raw MediaPipe result object.

        The result is typically parsed by the analyzer layer to extract relevant
        pose landmark data.
        ...
        Raises:
            ...
            ValueError: If frame_timestamp is not provided in "video" or
                "live_stream" modes.
        Returns:
            vision.PoseLandmarkerResult: Pose landmarker result object.
        """
        if image is None:
